...
Class Ingreso
{
	...
	{
		$sql="SELECT di.idingreso,di.idarticulo,a.nombre,a.descripcion,di.cantidad,di.precio_compra,di.precio_venta,i.tipo_comprobante,i.num_comprobante,c.nombre as categoria FROM detalle_ingreso di inner join articulo a on di.idarticulo=a.idarticulo inner join categoria c on a.idcategoria=c.idcategoria inner join ingreso i on i.idingreso=di.idingreso where di.idingreso='$idingreso'";
		return ejecutarConsulta($sql);
	}
