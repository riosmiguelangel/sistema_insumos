...
switch ($_GET["op"]){
	...
	case 'listarDetalle':
		...
		while ($reg = $rspta->fetch_object())
				{
					echo '<tr class="filas"><td>'.$reg->tipo_comprobante.'-'.$reg->num_comprobante.'</td><td>'.$reg->categoria.'</td><td>'.$reg->nombre.'</td><td>'.$reg->descripcion.'</td><td>'.$reg->cantidad.'</td></tr>';
	...
	case 'listarArticulos':
		...
		$rspta=$articulo->listarActivos();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>'<button class="btn btn-success" id="agregarDetalle'.$reg->idarticulo.'" onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\',\''.$reg->idcategoria.'\',\''.$reg->categoria.'\',\''.$reg->descripcion.'\')"><span class="fa fa-plus"></span></button>',
 				"1"=>$reg->nombre,
 				"2"=>$reg->categoria,
 				"3"=>$reg->codigo,
 				"4"=>$reg->stock,
