...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idpersona)){
			$rspta=$persona->insertar($tipo_persona,ucwords($nombre),$tipo_documento,str_pad($num_documento,8,0, STR_PAD_LEFT),ucwords($direccion),$telefono,$email);
			echo $rspta ? "Persona registrada" : "Persona no se pudo registrar";
		}
		else {
			$rspta=$persona->editar($idpersona,$tipo_persona,ucwords($nombre),$tipo_documento,$num_documento,ucwords($direccion),$telefono,$email);
		...
		$rspta=$persona->listarc();
 		...
 		while ($reg=$rspta->fetch_object()){
 			if (($_SESSION["cargo"])=="ADMIN"){
 				$data[]=array(
	 				"0"=>($reg->condicion)?'<button class="btn btn-primary" onclick="mostrar('.$reg->idpersona.')"><i class="fa fa-pencil"></i></button>'.
	 				...
	 				"1"=>ucwords($reg->tipo_documento),
	 				"2"=>ucwords($reg->tipo_documento.' '.$reg->nombre),
	 				"3"=>$reg->direccion,
	 				//"4"=>$reg->telefono,
	 				//"5"=>$reg->email
	 				);
